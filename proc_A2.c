...
int main(int argc, char ** argv){
	...
	if((RW_B1_sem_id = binary_semaphore_allocation(RW_B1_sem_key, 0666)) < 0){
		printf("A2 - Blad przy podlaczaniu do RW_B1_sem!\n");
		return 1;
	}
	...
	if((A1_sem_id = binary_semaphore_allocation(A1_sem_key, 0666)) < 0){
		printf("A3 - Blad przy podlaczaniu do A2.1_sem!\n");
		return 1;
	}
	if((A2_sem_id = binary_semaphore_allocation(A2_sem_key, 0666)) < 0){
		printf("A2 - Blad przy podlaczaniu do A2.2_sem!\n");
		return 1;
	}
	if((A3_sem_id = binary_semaphore_allocation(A3_sem_key, 0666)) < 0){
		printf("A3 - Blad przy podlaczaniu do A2.3_sem!\n");
		return 1;
	}
...
/*================================================================================================*/
	while(1){
		...
		if(queue_B1[0] > 0 && queue_B2[0] < Y){
			binary_semaphore_wait(A2_sem_id);
			item = queuePop(queue_B1);
			queueInsert(queue_B2, item + 1);
